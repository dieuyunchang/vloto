// Template updater module for Vietlot45
// Updates templates.json when new lottery data is added

const fs = require('fs');
const path = require('path');

// Function to convert a number to its group (G0-G4 for vietlot45)
...
function readExistingTemplates() {
    ...
    try {
        ...
        console.log('No existing templates file found for vietlot45');
        return [];
    }
}
...
function assignTemplateIds(lotteryData) {
    ...
    return lotteryData.map(entry => {
        if (entry.numbers) {
            const pattern = numbersToPattern(entry.numbers);
...
function updateTemplatesWithNewData(newLotteryData) {
    console.log('\n🔄 Updating templates with new vietlot45 data...');
    ...
    try {
        ...
        console.log(`📊 Processed ${processedCount} entries from vietlot45`);
        ...
        if (newPatterns.size > 0) {
            ...
            console.log(`✅ Added ${newTemplates.length} new templates for vietlot45`);
            ...
            console.log(`ℹ️  No new templates needed for vietlot45`);
        }
        
        // Always assign template IDs to the data after processing templates
        console.log(`🏷️  Assigning template IDs to vietlot45 entries...`);
        return assignTemplateIds(newLotteryData);
        
    } catch (error) {
        console.error(`❌ Error updating templates for vietlot45:`, error.message);
